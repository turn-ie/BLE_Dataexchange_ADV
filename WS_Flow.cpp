...
// （任意）旧ブロッキングRadar残置。共通色相使用。
void Radar_PlayOnce(){
  const float SPEED=2.5f; const float BW_F=0.8f; const float BW_B=0.05f; const uint8_t FADE=10; const uint16_t DT=16; const uint8_t HSTEP=24;
  ...
  while(ang<360.f){
    ...
    Matrix.show(); ang+=SPEED; if(ang>=360.f){ hue+=HSTEP; break;} delay(DT);
  }
  gMotionHue = hue; // 共有色相進行
  // フェードアウト簡略: 全消灯
...
void Ripple_PlayOnce(uint16_t){
  ...
  const float PERIOD=MAX_DIST+(RINGS-1)*SPACING+2.f*SIGMA; const uint8_t HSTEP=24;
  Matrix.setBrightness(gMotionBrightness);
  uint8_t localHue = gMotionHue; // 共有色相開始
...
// 非ブロッキング レーダー
static float sRadarAngleDeg=0.f; static bool sRadarActive=false; const float RADAR_SPEED=2.5f; const float BW_F=0.8f; const float BW_B=0.05f; const uint8_t FADE=10; const uint8_t HSTEP_IDLE=24;
